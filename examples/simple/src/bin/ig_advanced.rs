...
use lightstreamer_rs::subscription::{Subscription, SubscriptionListener, ItemUpdate, SubscriptionMode, Snapshot};
use lightstreamer_rs::connection::management::{ReconnectionConfig, HeartbeatConfig};
...
impl SubscriptionListener for AdvancedFinancialListener {
    fn on_item_update(&self, update: &ItemUpdate) {
        let item = serde_json::to_string_pretty(&update).unwrap_or_else(|_| "Invalid JSON".to_string());
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let config = Config {
        ...
        x_security_token: std::env::var("X_SECURITY_TOKEN").expect("X_SECURITY_TOKEN environment variable not set"),
        account_id: std::env::var("IG_ACCOUNT_ID").expect("IG_ACCOUNT_ID environment variable not set"),
    };
    
    info!("🔐 Loaded IG Markets authentication for account: {}", config.account_id);
    ...
    // Configure connection options and enable auto-reconnection
    {
        ...
        client_guard.connection_options.set_forced_transport(Some(Transport::WsStreaming));
        client_guard.connection_options.set_reconnect_timeout(3000).expect("Failed to set reconnect timeout");
        client_guard.connection_options.set_keepalive_interval(15000).expect("Failed to set keepalive interval");
        client_guard.connection_options.set_idle_timeout(60000).expect("Failed to set idle timeout");
        
         // Enable auto-reconnection with advanced configuration
         let _ = client_guard.enable_auto_reconnect_with_config(
             reconnection_config,
             heartbeat_config,
         );
    ...
    match client.lock().await.connect_direct(shutdown_signal.clone()).await {
        Ok(_) => {
...
async fn setup_financial_subscriptions(
    ...
    let mut market_subscription = Subscription::new(
        ...
        Some(vec!["BID".to_string(), "OFFER".to_string(), "HIGH".to_string(), "LOW".to_string()])
    ).unwrap();
    
    market_subscription.set_data_adapter(None).unwrap();
    market_subscription.set_requested_snapshot(Some(Snapshot::Yes)).unwrap();
    market_subscription.add_listener(Box::new(AdvancedFinancialListener::new("Market Data".to_string())));
     ...
     let mut account_subscription = Subscription::new(
         ...
         Some(vec!["PNL".to_string(), "AVAILABLE_CASH".to_string(), "EQUITY".to_string()])
     ).unwrap();
     
     account_subscription.set_data_adapter(None).unwrap();
     account_subscription.set_requested_snapshot(Some(Snapshot::Yes)).unwrap();
     account_subscription.add_listener(Box::new(AdvancedFinancialListener::new("Account Data".to_string())));
     
     // Add subscriptions to client and configure connection options
      {
          let mut client_guard = client.lock().await;
          LightstreamerClient::subscribe(client_guard.subscription_sender.clone(), market_subscription).await;
          LightstreamerClient::subscribe(client_guard.subscription_sender.clone(), account_subscription).await;
          
          // Configure connection options for financial data streaming
          client_guard.connection_options.set_forced_transport(Some(Transport::WsStreaming));
          client_guard.connection_options.set_reconnect_timeout(3000).expect("Failed to set reconnect timeout");
          client_guard.connection_options.set_keepalive_interval(30000).expect("Failed to set keepalive interval");
          client_guard.connection_options.set_idle_timeout(120000).expect("Failed to set idle timeout");
      }
...
async fn monitor_connection_metrics(_client: Arc<Mutex<LightstreamerClient>>, shutdown_signal: Arc<Notify>) {
