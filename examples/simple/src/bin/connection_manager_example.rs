...
impl SubscriptionListener for ConnectionAwareListener {
    fn on_item_update(&self, update: &ItemUpdate) {
        ...
        let fields = vec![
            "stock_name", "last_price", "time", "pct_change",
            "bid", "ask", "min", "max", "ref_price"
        ];
        ...
        info!("[{}] {}: {}", self.name.bright_blue(), item_name.bright_green(), output);
    }
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    info!("{}", "🚀 Starting ConnectionManager Example".bright_magenta().bold());
    info!("{}", "This example demonstrates advanced connection management features.".dimmed());
    ...
    info!("⚙️  Reconnection config: max_attempts={:?}, initial_delay={}s, max_delay={}s, multiplier={}, jitter={}",
    ...
    info!("💓 Heartbeat config: enabled={}, interval={}s, timeout={}s",
    ...
    // Enable auto-reconnection with custom configuration
    {
        ...
        client_guard.enable_auto_reconnect_with_config(
            reconnection_config,
            heartbeat_config,
        )?;
    ...
    // Add subscriptions to client
    {
        ...
        for subscription in subscriptions {
            LightstreamerClient::subscribe(client_guard.subscription_sender.clone(), subscription).await;
        }
    }
    
    info!("{}", "📡 Subscriptions added - they will be preserved during reconnections".bright_green());
    ...
    info!("{}", "🔌 Connecting with auto-reconnection enabled...".bright_cyan());
    
    let connect_result = LightstreamerClient::connect(
        Arc::clone(&client),
    ...
    match connect_result {
        Ok(_) => {
            ...
            info!("{}", "🕐 Running for 60 seconds to demonstrate reconnection features...".bright_blue());
            info!("{}", "   Try disconnecting your network to see auto-reconnection in action!".dimmed());
    ...
    // Display final metrics
    {
        ...
        info!(
            ...
            final_metrics.successful_reconnections.to_string().bright_green(),
            final_metrics.failed_reconnections.to_string().bright_red(),
            final_metrics.heartbeat_failures.to_string().bright_yellow()
        );
    }
    
    info!("{}", "✅ ConnectionManager example completed successfully!".bright_green().bold());
    info!("{}", "Key features demonstrated:".dimmed());
    info!("{}", "  • Automatic reconnection with exponential backoff".dimmed());
    info!("{}", "  • Heartbeat monitoring for connection health".dimmed());
    info!("{}", "  • Subscription preservation during reconnections".dimmed());
    info!("{}", "  • Real-time connection state and metrics monitoring".dimmed());
